...
// Normal addition
#if defined(OPENCL_NVIDIA) || defined(CUDA) || defined(HIP)
...
CONSTANT Fq Fq_ONE = { { 196605, 1980301312, 3289120770, 3958636555, 1405573306, 1598593111, 1884444485, 2010011731, 2723605613, 1543969431, 4202751123, 368467651 } };
CONSTANT Fq Fq_P = { { 4294945451, 3120496639, 2975072255, 514588670, 4138792484, 1731252896, 4085584575, 1685539716, 1129032919, 1260103606, 964683418, 436277738 } };
CONSTANT Fq Fq_R2 = { { 473175878, 4108263220, 164693233, 175564454, 1284880085, 2380613484, 2476573632, 1743489193, 3038352685, 2591637125, 2462770090, 295210981 } };
...
KERNEL void G2_bellman_multiexp(
 ...
 ///RR TODO
 ///Not used
 // KERNEL void kernel_mac_with_carry_64(ulong *a, ulong *b, ulong *c, ulong *d, ulong *result)  {
 //   *result = mac_with_carry_64(*a, *b, *c, d);
 // }
 ...
 KERNEL void kernel_Fr_gte(Fr *a, Fr *b, bool *result)  {
   *result = Fr_gte(*a, *b);
 }

 KERNEL void kernel_Fr_eq(Fr *a, Fr *b, bool *result)  {
   *result = Fr_eq(*a, *b);
 }

 KERNEL void kernel_Fr_sub(Fr *a, Fr *b, Fr *result)  {
   *result = Fr_sub(*a, *b);
 }

 KERNEL void kernel_Fr_add(Fr *a, Fr *b, Fr *result)  {
   *result = Fr_add(*a, *b);
 }

 KERNEL void kernel_Fr_reduce(uint32_t *accLow, uint32_t *np0, uint32_t *fq)  {
   Fr_reduce(accLow, *np0, fq);
 }

 KERNEL void kernel_Fr_mult_v1(uint32_t *x, uint32_t *y, uint32_t *xy) {
   Fr_mult_v1(x, y, xy);
 }

 KERNEL void kernel_Fr_mul_nvidia(Fr *a, Fr *b, Fr *result) {
   *result = Fr_mul_nvidia(*a, *b);
 }

 KERNEL void kernel_Fr_double(Fr *a, Fr *result) {
   *result = Fr_double(*a);
 }

 KERNEL void kernel_Fr_sqr(Fr *a, Fr *result) {
   *result = Fr_sqr(*a);
 }

 KERNEL void kernel_Fr_pow(Fr *base, uint *exponent, Fr *result) {
   *result = Fr_pow(*base, *exponent);
 }

 KERNEL void kernel_Fr_pow_lookup(Fr *bases, uint *exponent, Fr *result) {
   *result = Fr_pow_lookup(bases, *exponent);
 }

 KERNEL void kernel_Fr_mont(Fr *a, Fr *result) {
   *result = Fr_mont(*a);
 }

 KERNEL void kernel_Fr_unmont(Fr *a, Fr *result) {
   *result = Fr_unmont(*a);
 }

 KERNEL void kernel_Fr_get_bit(Fr *l, uint *i, bool *result) {
   *result = Fr_get_bit(*l, *i);
 }

 KERNEL void kernel_Fr_get_bits(Fr *l, uint *skip, uint *window, uint *result) {
   *result = Fr_get_bits(*l, *skip, *window);
 }

 KERNEL void kernel_Fq_sub_nvidia(Fq *a, Fq *b, Fq *result)  {
   *result = Fq_sub_nvidia(*a, *b);
 }

 KERNEL void kernel_Fq_add_nvidia(Fq *a, Fq *b, Fq *result)  {
   *result = Fq_add_nvidia(*a, *b);
 }

 KERNEL void kernel_Fq_gte(Fq *a, Fq *b, bool *result)  {
   *result = Fq_gte(*a, *b);
 }

 KERNEL void kernel_Fq_eq(Fq *a, Fq *b, bool *result)  {
   *result = Fq_eq(*a, *b);
 }

 KERNEL void kernel_Fq_sub(Fq *a, Fq *b, Fq *result)  {
   *result = Fq_sub(*a, *b);
 }

 KERNEL void kernel_Fq_add(Fq *a, Fq *b, Fq *result)  {
   *result = Fq_add(*a, *b);
 }

 KERNEL void kernel_Fq_reduce(uint32_t *accLow, uint32_t *np0, uint32_t *fq)  {
   Fq_reduce(accLow, *np0, fq);
 }

 KERNEL void kernel_Fq_double(Fq *a, Fq *result) {
   *result = Fq_double(*a);
 }

 KERNEL void kernel_Fq_sqr(Fq *a, Fq *result) {
   *result = Fq_sqr(*a);
 }

 KERNEL void kernel_Fq_pow(Fq *base, uint *exponent, Fq *result) {
   *result = Fq_pow(*base, *exponent);
 }

 KERNEL void kernel_Fq_pow_lookup(Fq *bases, uint *exponent, Fq *result) {
   *result = Fq_pow_lookup(bases, *exponent);
 }

 KERNEL void kernel_Fq_mont(Fq *a, Fq *result) {
   *result = Fq_mont(*a);
 }

 KERNEL void kernel_Fq_unmont(Fq *a, Fq *result) {
   *result = Fq_unmont(*a);
 }

 KERNEL void kernel_Fq_get_bit(Fq *l, uint *i, bool *result) {
   *result = Fq_get_bit(*l, *i);
 }

 KERNEL void kernel_Fq_get_bits(Fq *l, uint *skip, uint *window, uint *result) {
   *result = Fq_get_bits(*l, *skip, *window);
 }

 KERNEL void kernel_Fq_mult_v1(uint32_t *x, uint32_t *y, uint32_t *xy) {
   Fq_mult_v1(x, y, xy);
 }

 KERNEL void kernel_Fq_mul_nvidia(Fq *a, Fq *b, Fq *result) {
   *result = Fq_mul_nvidia(*a, *b);
 }

 KERNEL void kernel_Fq2_eq(Fq2 *a, Fq2 *b, bool *result) {
   *result = Fq2_eq(*a, *b);
 }

 KERNEL void kernel_Fq2_sub(Fq2 *a, Fq2 *b, Fq2 *result) {
   *result = Fq2_sub(*a, *b);
 }

 KERNEL void kernel_Fq2_add(Fq2 *a, Fq2 *b, Fq2 *result) {
   *result = Fq2_add(*a, *b);
 }

 KERNEL void kernel_Fq2_double(Fq2 *a, Fq2 *result) {
   *result = Fq2_double(*a);
 }

 KERNEL void kernel_Fq2_mul(Fq2 *a, Fq2 *b, Fq2 *result) {
   *result = Fq2_mul(*a, *b);
 }

 KERNEL void kernel_Fq2_sqr(Fq2 *a, Fq2 *result) {
   *result = Fq2_sqr(*a);
 }
 ...
 KERNEL void kernel_bitreverse(uint *n, uint *bits, uint *result) {
   *result = bitreverse(*n, *bits);
 }

// //reset carry(CC.CF flag) for test
// KERNEL void kernel_reset_carry_flag() {
//     uint32_t r=0;
// #if defined(CUDA)
//     asm volatile ("add.cc.u32 %0, %0, %1;" : "=r"(r) : "r"(0));
// #elif defined(HIP)
//     asm volatile ("v_add_co_u32 %0, %0, %1" : "+v"(r): "v"(r), "v"(r));
// #endif
// }
// #endif